...
def findDocumentsForQuery(title):
    ...
    if title['date'] != '':
        print("!!!!!!")
        content_date = db_class.executeAll("SELECT RID FROM testDB.content_date WHERE decade = '%d'" % int(title['date']))
        new_date = []
        for data in content_date:
            new_date.append(data['RID'])
    ...
    for term in searchkey:
        ...
        for doc in docDict:
            ...
            b=0.75
            ld0 = db_class.executeOne("SELECT tot_word_cnt, institution_name FROM testDB.research WHERE RID = '%d'" % doc[0])
            ld = ld0['tot_word_cnt']
            insti_name = ld0['institution_name']
            ...
            if(title['inst'] is not None and insti_name == title['inst']):
                BM25 = BM25 + 100
            if title['date'] !='' and doc[0] in new_date:
                BM25 = BM25 + 100
